...
import { Stage, Rect, Circle, Group, Line, calcLineLength } from '../components/render'
...
class RectWithCircle {
  ...
  private createRect(rectData) {
    this.rect = new Rect({ fillStyle: 'white', strokeStyle: '#444', lineWidth: 2, cornerRadius: 2, ...rectData })
  }

  private createCircles() {
    const { rect } = this

    const opt = { draggable: false, radius: 4, fillStyle: 'white', strokeStyle: 'blue', cursor: 'crosshair' } as const

    this.circles = [
      new Circle({ x: rect.data.x + rect.data.width / 2, y: rect.data.y, ...opt }),
      new Circle({ x: rect.data.x + rect.data.width, y: rect.data.y + rect.data.height / 2, ...opt }),
      new Circle({ x: rect.data.x + rect.data.width / 2, y: rect.data.y + rect.data.height, ...opt }),
      new Circle({ x: rect.data.x, y: rect.data.y + rect.data.height / 2, ...opt })
    ]
  ...
  static detectCircle(x: number, y: number, circles) {
    for (const item of circles) {
      if (calcLineLength({ x, y }, { x: item.data.x, y: item.data.y }) <= item.data.radius) {
        return item
      }
    }
...
function RectDragLine() {
  ...
  useEffect(() => {
    ...
    const rectWithCircles_1 = new RectWithCircle({ x: 100, y: 50, width: 100, height: 100 })
    const rectWithCircles_2 = new RectWithCircle({ x: 400, y: 50, width: 100, height: 100 })
    const rectWithCircles_3 = new RectWithCircle({ x: 250, y: 160, width: 100, height: 100 })
    ...
    stage.onmousemove = evt => {
      ...
      if (!line) {
        ...
        line.attr({ points: [downCircle.data.x, downCircle.data.y, evt.x, evt.y] })
      }

      other = downRectWithCircle.detectOther(evt.x, evt.y)
      if (other) {
        line.attr({
          points: [downCircle.data.x, downCircle.data.y, other.collisionCircle.data.x, other.collisionCircle.data.y]
        })
      }
    }
  ...
  // return (
  //   <Stage>
  //     <RectNode draggable controlStyle={{}} strokeStyle="#444" />
  //     <RectNode draggable controlStyle={{}} strokeStyle="#444" />
  //     <RectNode draggable controlStyle={{}} strokeStyle="#444" />
  //   </Stage>
  // )

  return <div className="canvas-container" ref={canvasRef} style={{ width: '100%', height: 400 }}></div>
}

export default RectDragLine
